...
public class ChouetteReferentialRestClient {
  ...
  public ChouetteReferentialRestClient(
    @Autowired WebClient.Builder webClientBuilder,
    @Value(
      "${chouette.rest.referential.base.url:http://chouette/referentials}"
    ) String chouetteRestServiceBaseUrl,
    @Value("${chouette.rest.referential.retry.max:3}") int maxRetryAttempts
  ) {
    this.webClient =
      webClientBuilder
        .baseUrl(chouetteRestServiceBaseUrl)
        .clientConnector(
  ...
  public void createReferential(ChouetteReferentialInfo referential) {
    try {
      exchangeForChouetteReferentialInfo(
        referential,
        HttpMethod.POST,
        "/create"
      );
      ...
      if (e.getStatusCode() == HttpStatus.CONFLICT) {
        LOGGER.warn(
          "The referential {} already exists in Chouette DB. Ignoring creation request",
          referential.getSchemaName()
        );
      } else {
        throw new ChouetteServiceException(
          "The Chouette referential service returned an error",
          e
        );
      }
    } catch (Exception e) {
      throw new ChouetteServiceException(
        "The Chouette referential service is unavailable",
        e
      );
    }
  }
  ...
  private Void exchangeForChouetteReferentialInfo(
    ChouetteReferentialInfo referential,
