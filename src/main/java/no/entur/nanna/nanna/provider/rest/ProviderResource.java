...
import no.entur.nanna.nanna.provider.domain.TransportMode;
...
@Path("")
@Tags(
  value = { @Tag(name = "ProviderResource", description = "Provider resource") }
)
public class ProviderResource {
  ...
  public Provider createProvider(Provider provider) {
    ...
    if (providerRepository.getProvider(referential) != null) {
      LOGGER.warn(
        "Failed to create provider {}: the provider already exists in the database",
        provider
      );
    ...
    try {
      ...
      throw new InternalServerErrorException(
        "Chouette Server error while creating the provider with id= " +
        provider.getId()
      );
    }
